...
export function WhatsAppFloatingButton({ phone }: Props) {
  ...
  return (
    <a
      ...
      className="fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-black/30 ring-2 ring-gold/25 transition hover:scale-105 hover:bg-[#20bd5a] hover:shadow-xl sm:bottom-6 sm:right-6 sm:h-[3.75rem] sm:w-[3.75rem]"
    >
...
export function WhatsAppContactLine({ phone }: Props) {
  const url = buildWhatsAppUrl(phone, BOOKING_MESSAGE);
  const display = formatWhatsAppDisplay(phone);
  if (!url || !display) return null;
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-2 text-gold transition hover:text-champagne"
    >
      <WhatsAppIcon className="h-4 w-4" />
      {display}
    </a>
